...
import { distXZ, clamp, damp } from './utils.js';
...
Game.prototype._buildLights = function () {
...
// ------------------------------------------------------------
Game.prototype.start = async function (name, room) {
...
// ------------------------------------------------------------
Game.prototype.onLocalHit = function (target, dmg, point) {
  this.hud.hitMarker();
  // hit feedback handled here; crime heat applied in onWeaponFired path
  if (target.kind === 'ped') {
...
Game.prototype.onWeaponFired = function (w) {
  // generic public gunfire raises a little heat + scares peds
  if (w.type === 'gun') {
...
Game.prototype.onWantedChanged = function (stars, up) {
...
Game.prototype.onCopHitPlayer = function (dmg) {
...
Game.prototype.onRemoteDamage = function (dmg, health, attackerId) {
...
Game.prototype.update = function (dt) {
  ...
  this.wanted.update(dt, seen || this.wanted.stars === 0 ? seen : false);
...
Game.prototype._handleMetaKeys = function () {
  if (this.input.justPressed('Escape')) {
    this.togglePause();
  }
  if (this.input.justPressed('Enter') && this.input.down('Shift')) {
    this._openChat();
  }
  // enter/exit vehicle
...
// On-foot update
// ------------------------------------------------------------
Game.prototype._updateFoot = function (dt) {
  const i = this.input;
...
Game.prototype._vehicleImpacts = function (v, dt) {
...
// Enter / exit vehicle
// ------------------------------------------------------------
Game.prototype._tryEnterVehicle = function () {
...
Game.prototype._enterVehicle = function (v) {
...
Game.prototype._exitVehicle = function () {
  const v = this.currentVehicle;
  if (!v) return;
  this.net.releaseVehicle(v.id);
  // place player beside the car
  const side = v.forward(tmpVec2).cross(new THREE.Vector3(0, 1, 0));
...
// ------------------------------------------------------------
Game.prototype._computeAim = function () {
...
// Time of day
// ------------------------------------------------------------
Game.prototype._updateTimeOfDay = function () {
  // map timeOfDay(0..1) to sun angle + color
...
// Chat (basic)
Game.prototype._openChat = function () {
